...
void MandelbrotScene::updateMandelbrotLine()
{
    std::vector<glm::highp_vec2> vertices = std::vector<glm::highp_vec2>();
    vertices.push_back(fromWorldToScreen(juliaPoint, bounds));

    glm::highp_vec2 z(0.0f, 0.0f);
    ...
    glBufferData(GL_ARRAY_BUFFER, lineVertexCount * sizeof(juliaPoint), &vertices[0], GL_DYNAMIC_DRAW);
...
void MandelbrotScene::handleEvents(const SDL_Event &event)
{
    switch (event.type)
    {
    case SDL_MOUSEBUTTONDOWN:
        if (event.button.button == SDL_BUTTON_LEFT)
        {
            ...
            pointGrabbed = glm::distance(realPosition, juliaPoint) < worldWidth / screenSize.x * 5;
        }
        break;
    ...
    case SDL_MOUSEMOTION:
        if (mouseDown)
        {
            ...
            if (pointGrabbed)
            {
                juliaPoint.x -= delta.x;
                juliaPoint.y += delta.y;
            }
            else
            {
                origin.x += delta.x;
...
void MandelbrotScene::update(const double dt)
{
}

void MandelbrotScene::refresh()
{
    ...
    updateMandelbrotLine();
}

void MandelbrotScene::reset()
{
...
void MandelbrotScene::render()
{
    ...
    auto screenPos = fromWorldToScreen(juliaPoint, bounds);
    glPointSize(7.0f);
    glBegin(GL_POINTS);
